...
> 思路：二分法。如果下一个数大于当前数，那么这个肯定是上坡，按照这条路肯定能得到峰值。
...
var compareVersion = function(version1, version2) {
  let v1Len = version1.length,
      v2Len = version2.length;
  let i1 = 0, i2 = 0;
  while (i1 < v1Len && i2 < v2Len) {
    let v1 = 0, v2 = 0;
    while (i1 < v1Len && version1[i1] !== ".")
      v1 = v1 * 10 + Number(version1[i1++]);
    while(i2 < v2Len && version2[i2] !== ".")
      v2 = v2 * 10 + Number(version2[i2++]);
    if (v1 > v2) return 1;
  ...
  if (i1 === v1Len) {
    while (i2 < v2Len) {
      if (version2[i2] !== "." && Number(version2[i2]) > 0)
        return -1;
      i2++;
    }
  } else {
    while (i1 < v1Len) {
      if (version1[i1] !== "." && Number(version1[i1] > 0))
        return 1;
      i1++;
    }
  }
...
BSTIterator.prototype.addQueue = function(node) {
  while(node !== null && node !== undefined) {
    this.queue.push(node);
    node = node.left;
  }
  this.node = this.queue.pop() || null;
}
...
> 思路：实际上就是冒泡排序, str(a) + str(a+1) 与 str(a+1) + str(a) 进行比大小
...
var largestNumber = function(nums) {
  ...
  for (let i = 0; i < nums.length - 1; i++) {
    for (let j = 0; j < nums.length - 1 - i; j++) {
      let t1 = Number("" + nums[j] + nums[j + 1]),
          t2 = Number("" + nums[j + 1] + nums[j]);
      if (t1 < t2) {
        let tmp = nums[j];
        nums[j] = nums[j + 1];
        nums[j + 1] = tmp;
...
class Trie {
  ...
  insert (word) {
    ...
    for (let i = 0; i < word.length; i++) {
      let tmpS = word[i]
      if (tmpNode[tmpS] === undefined)
        tmpNode[tmpS] = new TrieNode(tmpS)
      tmpNode[tmpS].num++;
      if (i === word.length - 1)
        tmpNode[tmpS].isEnd = true;
      tmpNode = tmpNode[tmpS].son;
    }
  };
   ...
   */
  search (word) {
    let tmpNode = this.root, len = word.length;
    for (let i = 0; i < len; i++) {
      let tmpS = word[i];
      if (tmpNode[tmpS] === undefined)
        return false;
      if (i === len - 1)
        return tmpNode[tmpS].isEnd
      tmpNode = tmpNode[tmpS].son;
    }
  };
  /**
   ...
   */
  startsWith (prefix) {
    let tmpNode = this.root, len = prefix.length;
    for (let i = 0; i < len; i++) {
      let tmpS = prefix[i];
      if (tmpNode[tmpS] === undefined)
        return false;
      if (i === len - 1)
        return tmpNode[tmpS].num > 0;
      tmpNode = tmpNode[tmpS].son;
    }
  };
}

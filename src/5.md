 ...
 */
class Solution  {
  private boolean[] usedEle;
  private int pIdx = 0;
  public TreeNode buildTree(int[] preorder, int[] inorder) {
    usedEle = new boolean[preorder.length];
    return handler(preorder, inorder, 0, preorder.length - 1);
  }

  private TreeNode handler(int[] preorder, int[] inorder, int iStart, int iEnd) {
    if (iStart > iEnd) return null;
    int pVal = preorder[pIdx++];
    int iIdx = indexOf(inorder, pVal);

    while (iIdx > -1 && usedEle[iIdx]) {
      pVal = preorder[pIdx++];
      iIdx = indexOf(inorder, pVal);
    }
    usedEle[iIdx] = true;
    TreeNode node = new TreeNode(pVal);

    node.left = handler(preorder, inorder, iStart, iIdx - 1);
    node.right = handler(preorder, inorder, iIdx + 1, iEnd);
